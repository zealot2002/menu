 ...
 * @author shaozuhceng
 */
public class EffectScrollView extends ScrollView {
	...
	public EffectScrollView(Context context, AttributeSet attrs) {
		super(context, attrs);
	}
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		...
		if (isTouchOutOfScrollView) { // 如果移动到了当前ScrollView控件之外
			if (isMoved) // 如果当前contentView已经被移动, 首先把布局移到原位置, 然后消费掉这个事件
				boundBack();
			return true;
		}
